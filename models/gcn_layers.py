...
class ResidualGatedGCNLayer(nn.Module):
    ...
    def __init__(self, hidden_dim, dropout, aggregation="sum"):
    ...
    def forward(self, x, e, edge_padding=None):
        """
        Args:
            x: Node features (batch_size, num_nodes, hidden_dim)
            e: Edge features (batch_size, num_nodes, num_nodes, hidden_dim)
            edge_padding: Edge padding (batch_size, num_nodes, num_nodes)
        ...
        if edge_padding != None:
            edge_gate = edge_gate.masked_fill(edge_padding.unsqueeze(-1), 0)
        # Node convolution
        ...
        x = F.relu(x_tmp)
        # Residual connection
        x_new = x_in + self.node_dropout(x)
        e_new = e_in + self.edge_dropout(e)
        return x_new, e_new
