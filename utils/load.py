...
class DataHub(object):
    ...
    def from_dir_addadj(cls, dir_path):
        ...
        house.data_collection["train"] = house._read_data(
            os.path.join(dir_path, "train.json"), True
        )
        house.data_collection["dev"] = house._read_data(
            os.path.join(dir_path, "dev.json"), False
        )
        house.data_collection["test"] = house._read_data(
    ...
    def _read_data(self,
        ...
        # 无法适配路径有多个点的情况 改为
        adjpath = os.path.splitext(file_path)[0]+"_adj.txt"
    ...
    def get_iterator(self, data_name, batch_size, shuffle,collate_func):
        data_set = _GeneralDataSet(*self.data_collection[data_name])
        data_loader = DataLoader(
            data_set, batch_size, shuffle, collate_fn=collate_func
        )
        return data_loader
